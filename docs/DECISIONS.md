...
## 3.5차: `scheduleVersion`을 도입한 이유

목표 시간을 바꾸면 미래 알림은 항상 새 시각으로 재조정되지만(`reconcile()`이 `lifeapp:goal-updated` 때마다
재실행됨), **이미 화면에 떠 있던 예전 알림**은 그 사실을 모른다. 사용자가 시각을 바꾸기 전에 온 알림을
나중에(다른 앱을 쓰다 알림 서랍에서) 눌러도 그게 "지금 기준으로 오래된 설정"인지 구분할 방법이 없었다.
`Goal.behavior.scheduleVersion`(기본 1)을 `behavior.time`/`behavior.timezone`이 실제로 바뀔 때만
올리고, 모든 알림이 예약 시점의 버전을 `extra`에 실어서, 액션을 받을 때 현재 버전과 비교해 다르면
GoalExecution을 건드리지 않고 배너로 안내하는 방식을 택했다. 제목/메모 등 스케줄과 무관한 변경은 버전을
안 올려서 불필요하게 알림을 "오래된 것"으로 만들지 않는다. 검증은 클라이언트(어댑터)에서만 하고
서버(`routes/goalExecutions.js`)의 상태 전이 검증 로직은 그대로 둔다 — 실수로 오래된 알림을 누르는 것을
막는 게 목적이라 이 정도로 충분하다고 판단했다(자세한 스키마는 `docs/ANDROID_ARCHITECTURE.md`).

## 3.5차: 오프라인 알림 액션을 `localStorage` 큐로만 처리한 이유

알림 액션을 받았는데 네트워크가 없으면 기존 코드는 그냥 `console.error`만 남기고 사용자의 선택을
잃어버렸다. 완전한 해법(백그라운드 동기화 서비스, 네이티브 재시도 스케줄러 등)은 이번 프로토타입 단계에
비해 과도하다고 판단해서, **액션을 네트워크 호출 전에 먼저 `localStorage`에 기록하고, 서버가 확정
응답을 준 뒤에만 지우는 최소 큐**로 구현했다. 새 Capacitor 플러그인(`@capacitor/preferences` 등) 추가
없이 WebView에 이미 있는 `localStorage`만 썼다. 재시도는 앱 시작/재개 시점에만 일어나고(백그라운드
주기적 재시도 없음), 5회 넘게 실패하면 자동 재시도를 멈추고 배너로 알린다 — 조용히 버리지 않는 것이
핵심이라 "몇 번까지 자동으로, 이후엔 사용자에게 알림" 정도로 충분하다고 봤다. `GET /today`/`PATCH
/transition`이 이미 멱등하므로 재시도로 인한 중복 반영 위험은 없다.

## 3.5차: 스누즈 기본값(5분) 유지, "오늘 건너뛰기" 알림 액션 UX는 보류

**스누즈 기본값**: 5분 유지 vs 10분으로 변경을 비교했다. 절전 모드(Doze)에서 정확히 5분 뒤에 안 올 수
있다는 위험은 실재하지만(값을 키운다고 근본적으로 없어지는 문제는 아니고, 상대적 체감 오차만 줄어드는
것), "빨리 다시 물어봐 주길" 기대하는 제품 의도와는 값을 키우는 쪽이 어긋난다고 판단해 **5분을 그대로
유지**하기로 했다. 대신 알림 문구를 "약 5분 후"처럼 근사 표현으로 두고 과도한 정확성을 약속하지 않는다.
실기기 테스트로 실제 지연 정도가 확인되면 재논의한다.

**"오늘 건너뛰기" 알림 액션**: 상태표상 `skipped`는 터미널이라 되돌릴 수 없는데, 알림에서 한 번의 실수
탭으로 바로 적용되는 게 위험하다고 판단했다. 앱을 열어 개입 모달에서 한 번 더 확인받는 쪽을 권장했지만,
이건 실제 UX 동작을 바꾸는 결정이라 **명시적 승인 없이는 반영하지 않고 현행(알림에서 즉시 적용) 유지**로
남겨뒀다. 승인되면 별도로 반영한다.

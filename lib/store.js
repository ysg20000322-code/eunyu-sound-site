...
async function mergeSeed(name, seedItems, idKey = "id") {
  const list = await readJSON(name, []);
  const existingIds = new Set(list.map((item) => item[idKey]));
  const newItems = seedItems.filter((item) => !existingIds.has(item[idKey]));
  if (newItems.length > 0) {
    await writeJSON(name, [...list, ...newItems]);
  }
}

// Serializes read-modify-write operations against the same resource name so
// concurrent requests handled by the same process don't clobber each other.
// Note: this does not protect against races across separate serverless
// instances — see limitations in the review-import feature notes.
const locks = new Map();

function withLock(name, fn) {
  const previous = locks.get(name) || Promise.resolve();
  const result = previous.then(fn, fn);
  locks.set(name, result.then(() => {}, () => {}));
  return result;
}

module.exports = { readJSON, writeJSON, seedIfMissing, mergeSeed, withLock };
